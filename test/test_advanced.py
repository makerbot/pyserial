...
class Test_ChangeAttributes(unittest.TestCase):
    ...
    def test_PortSetting(self):
        ...
        try:
            ...
            self.assertTrue(self.s.isOpen())
            self.assertEqual(self.s.port, 0)
            self.assertEqual(self.s.portstr, serial.device(0))
        try:
            self.s.port = 1
        except serial.SerialException: # port not available on system
            pass        # can't test on this machine...
            ...
            self.assertTrue(self.s.isOpen())
            self.assertEqual(self.s.port, 1)
            self.assertEqual(self.s.portstr, serial.device(1))
    ...
    def test_BaudrateSetting(self):
        ...
        for illegal_value in (-300, -1, 'a', None):
            self.assertRaises(ValueError, self.s.setBaudrate, illegal_value)
    ...
    def test_BytesizeSetting(self):
        ...
        for illegal_value in (0, 1, 3, 4, 9, 10, 'a', None):
            self.assertRaises(ValueError, self.s.setByteSize, illegal_value)
    ...
    def test_ParitySetting(self):
        ...
        for illegal_value in (0, 57, 'a', None):
            self.assertRaises(ValueError, self.s.setParity, illegal_value)
    ...
    def test_StopbitsSetting(self):
        ...
        for illegal_value in (0, 3, 2.5, 57, 'a', None):
            self.assertRaises(ValueError, self.s.setStopbits, illegal_value)
    ...
    def test_TimeoutSetting(self):
        ...
        for illegal_value in (-1, 'a'):
            self.assertRaises(ValueError, self.s.setTimeout, illegal_value)
